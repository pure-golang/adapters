...
Адаптер поддерживает выполнение команд на удалённых серверах через SSH.

#### Параметры SSH-конфигурации .env

| Переменная окружения | Описание | Обязательный | По умолчанию |
|---------------------|----------|--------------|--------------|
| `SSH_HOST` | Хост удалённого сервера | Нет* | - |
| `SSH_PORT` | Порт SSH | Нет | 22 |
| `SSH_USER` | Имя пользователя | Нет | - |
| `SSH_KEY_PATH` | Путь к приватному ключу | Нет | - |
| `SSH_PASSWORD` | Пароль для аутентификации | Нет | - |

\* Хотя бы один метод аутентификации (`SSH_KEY_PATH` или `SSH_PASSWORD`) должен быть указан.

> **Внимание**: Для аутентификации по паролю требуется установленный `sshpass`. Метод с ключом предпочтительнее для production-среды.
...
cfg := cli.Config{
    Command: "ffmpeg",
    // Альтернативная прямая конфигурация (без .env)
    SSH: cli.SSHConfig{
...
if err := executor.Start(); err != nil {
    log.Fatalf("Ошибка инициализации SSH: %v", err)
}

ctx := context.Background()
// Команда выполнится на удалённом сервере
err := executor.Execute(ctx, "-version")
```
